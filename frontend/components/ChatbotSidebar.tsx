...
const ChatSidePanel: React.FC<ChatSidePanelProps> = ({ isOpen, onClose }) => {
  ...
  const fetchGeminiResponse = async (userMessage: string) => {
    try {
      ...
      setMessages(prev => [...prev, { text: "Error contacting Gemini API.", sender: "ai" }]);
    }
  };
  ...
  const speak = (text: string) => {
    if ("speechSynthesis" in window) {
      ...
      if (!selectedVoice) selectedVoice = voices.find(v => v.lang === "en-US" && v.name.toLowerCase().includes("google"));
      if (!selectedVoice) selectedVoice = voices.find(v => v.lang === "en-US" && v.name.toLowerCase().includes("enhanced"));
      if (!selectedVoice) selectedVoice = voices.find(v => v.lang === "en-US" && v.name.toLowerCase().includes("female"));
      ...
      <div className="p-4 border-t flex items-center">
        ...
        <button
          className={`flex items-center gap-2 px-3 py-2 border-l border-r transition-colors duration-200
            ${listening ? "bg-red-500 text-white slow-pulse" : "bg-gray-200 text-black"}`}
        ...
        >
          ...
          {listening && <span className="ml-1 font-semibold slow-pulse">...</span>}
        </button>
